...
contaminant_choice = int(input("Enter the number of the contaminant for plotting polynomial regression:"))
...
model.fit(X_poly, df[contaminant_column])
...
# Transform 2022 features into polynomial features
X_2022_poly = poly.transform(df_2022[features])

# Use the trained model to make predictions for 2022
predictions_2022 = model.predict(X_2022_poly)
...
plt.title(f'Actual vs Predicted {contaminant_column} (2022) - Polynomial Regression (Degree {degree})')
plt.show()
